...
begin
  ...
  try
    ...
    try
      oQuery.Connection := FConn;
      oQuery.SQL.Text := 'UPDATE Pedidos SET ValorTotal = (SELECT COALESCE(SUM(ValorTotal),0) FROM PedidoProdutos WHERE NumeroPedido = :pNumeroPedido) WHERE NumeroPedido = :pNumeroPedido';
      oQuery.ParamByName('pNumeroPedido').AsInteger := Result;
      oQuery.ExecSQL;
    finally
      oQuery.Free;
  ...
  try
    ...
    try
      ...
      oQuery.SQL.Text := 'UPDATE Pedidos SET ValorTotal = '
                       ...
                       + ' WHERE NumeroPedido = :pNumeroPedido) WHERE NumeroPedido = :pNumeroPedido';
      oQuery.ParamByName('pNumeroPedido').AsInteger := APedido.NumeroPedido;
      oQuery.ExecSQL;
    finally
      oQuery.Free;
...
begin
  ...
  try
    ...
    try
      oQuery.Connection := FConn;
      oQuery.SQL.Text := 'DELETE FROM Pedidos WHERE NumeroPedido = :pNumeroPedido'; // ON DELETE CASCADE apaga itens
      oQuery.ParamByName('pNumeroPedido').AsInteger := ANumero;
      oQuery.ExecSQL;
    finally
      oQuery.Free;
